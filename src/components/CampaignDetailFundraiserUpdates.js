...
const CampaignDetailUpdates =()=>{
    ...
    useEffect(()=>{
        Axios
        .get(`https://binar8-agus-saputra.nandaworks.com/campaigns/update?campaignId=${campaignId}`)
        .then(response=>{
        ...
        <div className="custom-card campaign-updates my-5 py-4 px-3">
            <div className="container">
                ...
                <Timeline>
                    ...
                    <TimelineEvent
                        title="-"
                        createdAt={campaignUpdate.createdAt}
                    >
                        <div>{campaignUpdate.withdrawalAmount}</div>
                        <div> Withdrawal purpose: {campaignUpdate.withdrawalPurpose}</div>
                    </TimelineEvent>
                    ))}
                </Timeline>
                </div>
